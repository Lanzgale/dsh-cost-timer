/**
 * dsh-cost-timer — 平价闹钟(host 侧)。
 *
 * 原理(见会话设计):DSH 的 agent 是消息驱动的单步循环,"让某会话做事"的唯一入口
 * 是给它投一条用户消息。本模块 = 宿主看表:平价时段(谷价,约为峰价一半)开始时,
 ...
 *  - 周末(周六/周日,北京日历日,2026-08-23 新规):全天平价;
 *  - 工作日:高峰 09:00–12:00 与 14:00–18:00,其余平价。
 */
...
const DEFAULT_TEXT = [
  '【dsh-cost-timer · 平价提醒】当前处于 DeepSeek 平价时段(费用约为高峰的一半),适合处理积压工作。',
  '',
