 ...
 *     且必须是「非空白会话」——新会话页(新建未发言)没有上下文,不作为提醒目标;
 *  2. 边沿触发:轮询检测进入新低谷段(工作日 12:00 / 18:00,周末全天)时,目标
 *     live 且非空白 → 投递(每段一次,state 去重);
 *  3. 打开补发:监听 agent/created——低谷时段新打开(变为 live)的非空白会话若成为
 *     当前目标且本段未投递,立即补投。
 * 高峰段不投(省钱原则),等下一个低谷段。
 * 注意:保存配置(改提醒文案/拨开关)不触发任何投递,提醒只有上面 2、3 两个入口。
 *
...
export function installCostTimerAlarm(ctx) {
  ...
  /** 低谷段内对「当前打开的会话」投递(若 live、非空白会话,且本段未投)。 */
  async function dispatchCurrent(segmentKey, now) {
    ...
    if (await isBlankSession(ctx, agent.id)) return // 新会话页不提醒
  ...
  // 打开补发:低谷段新打开(成为 live 根)、非空白、且本段未投 → 补投。
  const onAgentCreated = (payload) => {
    ...
    setTimeout(() => {
      void (async () => {
        if (disposed || !cfg.enabled) return
        if (state.last[agent.id] === segment) return
        if (await isBlankSession(ctx, agent.id)) return // 新会话页不补发
        await deliver(agent, segment)
      })()
    }, 1200)
  }
