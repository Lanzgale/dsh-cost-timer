...
export function apply(ctx) {
  ...
  // 低谷闹钟:宿主看表 + 打开补发(配置/状态独立存储,不碰 ledger 严格 codec)。
  try {
    const alarm = installCostTimerAlarm(ctx)
    // 启动信息合并为一行:账本 + 闹钟数据目录/开关(原两行提示合并)。
    console.log(`[dsh-cost-timer] 已加载,账本:${ledger.path};低谷闹钟(数据目录 ${alarm.dataDir};enabled=${alarm.enabled})`)
  } catch (error) {
    console.warn('[dsh-cost-timer] 低谷闹钟安装失败: ' + String((error && error.message) || error))
    console.log(`[dsh-cost-timer] 已加载,账本:${ledger.path}`)
  }
}
