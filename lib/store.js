...
// Coding plan 已知提供商列表(dsh-cost-lite 已裁剪 coding plan 功能,仅用于
...
export class Ledger {
    ...
    try {
      ...
      if (parsed !== null && typeof parsed === 'object') {
        if (parsed.version !== LEDGER_VERSION) {
          console.warn(`[dsh-cost-lite] 账本版本 ${String(parsed.version)} 不受支持,按空账本启动`)
        } else {
      ...
      if (error?.code !== 'ENOENT') {
        console.warn(`[dsh-cost-lite] 账本读取失败,按空账本启动: ${String(error?.message ?? error)}`)
      }
    }
  ...
  flush() {
    ...
    try {
      ...
      console.warn(`[dsh-cost-lite] 账本写入失败: ${String(error?.message ?? error)}`)
    }
  }
