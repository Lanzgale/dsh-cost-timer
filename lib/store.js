...
// Coding plan 已知提供商列表(dsh-cost-timer 已裁剪 coding plan 功能,仅用于
// 兼容清洗旧配置中的残留字段;原来源模块 coding-plans.js 已移除)。
const CODING_PLAN_PROVIDER_IDS = ['anthropic', 'zai', 'minimax', 'kimi', 'openrouter', 'siliconflow', 'commandcode', 'scnet']
...
/**
 * 账本默认路径：`$DSH_HOME/storages/cost-meter/ledger.json`。
 * 路径只随 DSH_HOME 变化(无独立环境变量覆盖);导出供启动日志判断"是否位于非默认位置"。
 * @returns 账本文件的绝对路径。
 */
export function defaultLedgerPath() {
  return join(resolveDshHome(), 'storages', 'cost-meter', 'ledger.json')
}
...
export class Ledger {
  ...
  constructor(config, days, path, migrations = []) {
    ...
    // 上次充值后的余额基准({ total, granted, topped, currency, at }):**不随日期重置**,
    // 只在下一次充值/授信(分项变大)时重新充满,供卡片余额条显示"当前余额 / 充值后余额"。
    this.rechargeRef = null
  ...
  static open() {
    ...
    let days = {}
    let balanceRef = null
    let rechargeRef = null
    let migrations = []
    try {
      const parsed = JSON.parse(readFileSync(path, 'utf8'))
      if (parsed !== null && typeof parsed === 'object') {
        if (parsed.version !== LEDGER_VERSION) {
          console.warn(`[dsh-cost-timer] 账本版本 ${String(parsed.version)} 不受支持,按空账本启动`)
        } else {
          ...
          // 充值基准(卡片余额条):形状不对则丢弃,下次拉取余额时重新充满。
          const recharge = parsed.rechargeRef
          if (recharge !== null && typeof recharge === 'object'
            && Number.isFinite(recharge.total) && Number.isFinite(recharge.granted) && Number.isFinite(recharge.topped)) {
            rechargeRef = { total: recharge.total, granted: recharge.granted, topped: recharge.topped, currency: typeof recharge.currency === 'string' ? recharge.currency : undefined, at: Number(recharge.at) || 0 }
          }
        }
      }
    } catch (error) {
      if (error?.code !== 'ENOENT') {
        console.warn(`[dsh-cost-timer] 账本读取失败,按空账本启动: ${String(error?.message ?? error)}`)
      }
    }
    const ledger = new Ledger(config, days, path, migrations)
    ledger.balanceRef = balanceRef
    ledger.rechargeRef = rechargeRef
    return ledger
  }
  ...
  flush() {
    ...
    try {
      ...
      writeFileSync(tmp, JSON.stringify({ version: LEDGER_VERSION, config: this.config, days: this.days, balanceRef: this.balanceRef ?? null, rechargeRef: this.rechargeRef ?? null, migrations: this.migrations }), 'utf8')
      renameSync(tmp, this.path)
    } catch (error) {
      console.warn(`[dsh-cost-timer] 账本写入失败: ${String(error?.message ?? error)}`)
    }
  }
...
/**
 * 充值基准(卡片余额条,2026-09-17 改版):满格 = 上次充值后的余额,之后只随消费流失,
 * **不随日期重置**;仅充值/授信(分项变大)、总额回升、币种切换时重新充满。
 * 与对账用的 balanceRef 相互独立——后者必须保留当日基准用于 drift 检测,不能共用。
 * @param prevRef - 上一次充值基准({ total, granted, topped, currency, at });无则 null。
 * @param balance - 本次拉取的余额({ totalBalance, grantedBalance, toppedUpBalance, currency })。
 * @param nowMs - 当前时间戳,写入基准的 at。
 * @returns 下一次充值基准;余额不可用时原样返回。
 */
export function reconcileRechargeRef(prevRef, balance, nowMs) {
  if (balance === null || typeof balance !== 'object' || !Number.isFinite(balance.totalBalance)) {
    return prevRef ?? null
  }
  const snap = {
  ...
  if (prevRef === null || typeof prevRef !== 'object') return snap
  // 币种切换后金额不可比:重新充满。
  if (prevRef.currency !== snap.currency) return snap
  // 充值/授信:分项变大才能涨余额 → 电池回满。
  if (snap.granted > prevRef.granted + 0.009 || snap.topped > prevRef.topped + 0.009) return snap
  // 总额回升(退款/口径调整,非分项增加):同样回满,避免条超出满格。
  if (snap.total > prevRef.total + 0.009) return snap
  return prevRef
}
