...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      // 平价闹钟折叠区(浮窗内独立折叠栏)。
      '.cm-alarm-body{display:flex;flex-direction:column;gap:8px;padding-top:4px}',
      '.cm-alarm-row{display:flex;align-items:center;gap:8px;min-width:0;font-size:12px;color:var(--dsw-alias-label-secondary)}',
      '.cm-alarm-row input[type=checkbox]{flex:none;accent-color:var(--dsw-alias-state-business-primary,#4d6bfe)}',
      '.cm-alarm-name{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--dsw-alias-label-primary)}',
      '.cm-alarm-badge{flex:none;font-size:11px;color:var(--dsw-alias-label-tertiary)}',
      ...
      '.cm-alarm-text{flex:1;min-width:0;width:100%;box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);color:var(--dsw-alias-label-primary);border-radius:6px;padding:6px 8px;font-size:12px;line-height:1.6;font-family:var(--ds-font-family-code,monospace);outline:none;resize:vertical}',
      '.cm-alarm-text:focus-visible{border-color:var(--dsw-alias-state-business-primary)}',
      '.cm-alarm-text-block{flex-direction:column;align-items:stretch}',
    ...
    const MESSAGES = {
      zh: {
        ...
        // 平价闹钟
        alarmTitle: '平价闹钟',
        alarmSub: '平价时段开始时,自动为当前打开的会话发送提醒,可用于处理待办任务',
        alarmMaster: '启用提醒',
        alarmPath: '提醒内容(进入平价时段发送)',
        alarmLive: '打开中',
        alarmCurrent: '当前会话',
        alarmNoCurrent: '暂无打开中的会话',
        alarmPhaseOff: '当前 · 平价{seg}',
        alarmPhasePeak: '当前 · 高峰(平价时提醒)',
        alarmErr: '平价闹钟出错:{message}',
        // 费用统计(合并口径)
        statsTitle: '费用统计',
        statsMode: '口径',
        statsModeDate: '按日期统计',
      ...
      en: {
        ...
        alarmTitle: 'Off-peak alarm',
        alarmSub: 'At each off-peak start, automatically remind the currently open conversation (great for clearing backlog work)',
        alarmMaster: 'Enable off-peak reminders',
        alarmPath: 'Reminder text (sent at off-peak)',
        alarmLive: 'open',
        alarmCurrent: 'Current session',
        alarmNoCurrent: 'No open session now',
        alarmPhaseOff: 'Now · off-peak{seg}',
        alarmPhasePeak: 'Now · peak (remind at off-peak)',
        alarmErr: 'Off-peak alarm error: {message}',
    ...
    const CONTRIBUTION = {
      ...
      descriptors: [
        ...
        {
          id: 'dsh-cost-timer#costMeter/getTopSessions', service: 'costMeter', namespace: 'costMeter', method: 'getTopSessions',
          invocation: { kind: 'direct' },
          parameters: [
            { name: 'limit', wire: 'limit', source: 'json', codec: { mode: 'strict', typeSymbol: 'dsh-cost-timer#SessionLimit', schema: limitCodec } },
            { name: 'sort', wire: 'sort', source: 'json', codec: { mode: 'strict', typeSymbol: 'dsh-cost-timer#SessionSort', schema: sortCodec }, acceptsUndefined: true },
            { name: 'dir', wire: 'dir', source: 'json', codec: { mode: 'strict', typeSymbol: 'dsh-cost-timer#SessionSortDir', schema: sortCodec }, acceptsUndefined: true },
          ],
          result: { mode: 'strict', typeSymbol: 'dsh-cost-timer#TopSessions', schema: topSessionsCodec },
        },
      ],
    }
    ...
    // ── 平价闹钟(浮窗内独立折叠栏;配置/状态走同源 alarm 路由,不占用 ledger codec) ──
    const ALARM_API = '/plugins/dsh-cost-timer/alarm'

    function AlarmPanel(props) {
      const t = props.t
      const [open, setOpen] = useState(false)
      const [data, setData] = useState(null) // overview
      const [err, setErr] = useState('')
      const [busy, setBusy] = useState(false)
      const [text, setText] = useState('')
      // 文本框是否被用户改过且尚未确认保存:防止 30s 自动刷新把正在编辑的内容覆盖回去。
      const dirtyRef = useRef(false)
      // 待发送配置补丁(开关/文本交错时也只保留最新一次;发送完成后自动续发,避免丢改)。
      ...
      const load = async () => {
        try {
          ...
          // 有未保存草稿时不回填文本(编辑不丢;保存成功后 dirtyRef 复位再同步)。
          if (!dirtyRef.current) setText(body.overview.text)
          setErr('')
        } catch (e) {
          setErr(t('alarmErr', { message: e && e.message ? e.message : String(e) }))
        }
      }
      useEffect(() => { void load() }, [])
      // 打开时每 30s 轻量刷新(live 徽标/时段变化),关闭即停。
      useEffect(() => {
      ...
      // post 只入队,flush 负责串行发送:忙时不再丢补丁,开关/文本交错都能收敛到最终态。
      const post = (patch) => {
        queuedRef.current = patch
      ...
      const flush = async () => {
        ...
        try {
          while (queuedRef.current) {
            const patch = queuedRef.current
            queuedRef.current = null
            try {
              const res = await fetch(ALARM_API + '/config', {
                method: 'POST',
              ...
              if (patch && typeof patch === 'object' && Object.prototype.hasOwnProperty.call(patch, 'text')) {
                // 本次保存的正是提醒文本:回显即用户刚保存的版本,解除 dirty 后自动刷新可安全同步。
                dirtyRef.current = false
                setText(body.overview.text)
              }
              setErr('')
            } catch (e) {
              setErr(t('alarmErr', { message: e && e.message ? e.message : String(e) }))
              break // 单次失败即停,等下一次用户操作重新触发
            }
          }
        } finally {
      ...
      const phaseText = phase === null ? ''
        : (phase.offpeak ? t('alarmPhaseOff', { seg: phase.segmentLabel ? ' · ' + phase.segmentLabel : '' })
          : t('alarmPhasePeak'))
      ...
      const head = el('div', { className: 'cm-budget-head' },
        ...
        phaseText ? el('span', { className: 'cm-alarm-phase' + (phase && phase.offpeak ? ' off' : '') }, phaseText) : null)

      const enabled = data ? !!data.enabled : false
      const current = data && data.current ? data.current : null
      return el('div', { className: 'cm-budget' }, head,
        open ? el('div', { className: 'cm-collapse-body cm-alarm-body' },
          err ? el('p', { className: 'cm-msg warn' }, err) : null,
          el('p', { className: 'cm-alarm-note' }, t('alarmSub')),
          el('label', { className: 'cm-alarm-row' },
            el('input', { type: 'checkbox', checked: enabled, disabled: !data || busy, onChange: e => void post({ enabled: e.target.checked }) }),
            el('span', { className: 'cm-alarm-name' }, t('alarmMaster'))),
          el('div', { className: 'cm-alarm-row cm-alarm-text-block' },
            el('span', { className: 'cm-alarm-badge' }, t('alarmPath')),
            el('textarea', { className: 'cm-alarm-text', rows: 5, value: text, disabled: !data || busy,
              onChange: e => { setText(e.target.value); dirtyRef.current = true },
              onBlur: () => {
                if (!data) return
                if (text !== data.text) post({ text })
                else dirtyRef.current = false // 与服务器一致,解除草稿标记
              } })),
          el('div', { className: 'cm-alarm-row' },
            el('span', { className: 'cm-alarm-badge' }, t('alarmCurrent')),
            current
              ? el('span', { className: 'cm-alarm-name' }, (current.title || current.id) + ' · ' + t('alarmLive'))
              : el('span', { className: 'cm-alarm-name cm-sess-id' }, t('alarmNoCurrent')))) : null)
    }
    ...
    async function apply(ctx) {
      ...
      const registerSession = position => {
        ...
        const options = position === 'header'
          ? { name: slotName, id: 'cost-timer', order: -5, inject: injected }
          : { name: slotName, id: 'cost-timer', order: 5, inject: injected }
