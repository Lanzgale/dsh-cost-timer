...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SidebarFooter(props) {
      ...
      // 合并卡片(四元素一体,整卡为点击入口):行1 余额+今日,行2 今日本会话,行3 时间条。
      ...
      return el(Fragment, null,
        el('div', { ref: rootRef, className: 'cm-footer-stack' + (wide ? '' : ' rail') },
          el('div', {
            ...
            wide
              ? el(Fragment, null,
                (balanceItem !== null || todayItem !== null)
                  ? el('div', { className: 'cm-foot-card-row' }, balanceItem, todayItem)
                  : null,
                sessionItem !== null ? el('div', { className: 'cm-foot-card-row' }, sessionItem) : null,
    ...
    function CostSection(props) {
      ...
      // 自动保存状态:合并页仅剩「余额对账」开关会改配置,反馈放右上角。
      ...
      // 单页展示(概览 + 用量合并,无标签;价格页已裁剪)。
      return el('div', { className: 'cm-section' },
        el('div', { className: 'cm-toolbar' }, saveBadge),
        // 操作结果提示(清除历史):全局展示。
